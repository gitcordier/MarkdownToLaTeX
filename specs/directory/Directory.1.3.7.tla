...
(*****************************************************************************
  ...
  Subject.   The User runs MarkdownToLaTeX 1.0.0 from a working directory
             DIR. Before any conversion can begin, MarkdownToLaTeX
             decides whether DIR is "ready" at the file-system layer.
             Three flags carry the evidence:
                  1. is_dir
                     $DIR exists and is a directory;
                  2. has_preferences_dir
                     $DIR/preferences exists and is a directory;
                  3. has_JSON_preferences
                     $DIR/preferences/preferences.json exists and is a
                     regular file.
  ...
  Two records, two semantics.
             `directory : Knowledge` is the *trial verdict*. Each flag is
                         TRUE unless and until a check disproves it.
                         Init: all TRUE (no disproof yet). Transitions
                         flip TRUE -> FALSE only and never back.
             `check     : Knowledge` is the *inquiry log*. Each flag is
                         TRUE iff the corresponding check has been run.
                         Init: all FALSE. Transitions flip FALSE -> TRUE
                         only and never back.
             The two records share the structural type `Knowledge`; the
             distinct meanings live in the actions, not the type.
  ...
  Liveness.  This module proves SAFETY only. No fairness clause is
             added, no WF, no liveness theorem. Termination of the chain
             is not proven here (and is not needed for the headline XOR
             property since XOR reduces to disjointness under the
             definitions of OK and NOT_OK).
...
isTrue(n) ==
    CASE n = 1 -> directory.is_dir
      ...
      [] n = 3 -> directory.has_JSON_preferences
      [] OTHER -> FALSE
...
hasBeenChecked(n) ==
    CASE n = 1 -> check.is_dir
      ...
      [] n = 3 -> check.has_JSON_preferences
      [] OTHER -> FALSE

AllChecked            == \A n \in 1..3 : hasBeenChecked(n)
hasNotBeenChecked(n)  == ~ hasBeenChecked(n)
NothingHasBeenChecked == \A n \in 1..3 : hasNotBeenChecked(n)
...
InitDirectory ==
  ...
  /\ directory.has_JSON_preferences = TRUE

InitCheck ==
  /\ check.is_dir               = FALSE
  /\ check.has_preferences_dir  = FALSE
  /\ check.has_JSON_preferences = FALSE
...
(*  Every action requires NothingHasBeenDisproved as part of its guard, so   *)
...
(* Step 3: DIR/preferences/preferences.json is a regular file. *)
NecessaryJsonPreferences ==
  /\ hasBeenChecked(1)
  /\ hasBeenChecked(2)
  /\ hasNotBeenChecked(3)
  /\ NothingHasBeenDisproved
...
NextHasJsonPreferences ==
  /\ NecessaryJsonPreferences
  /\ UNCHANGED directory
  /\ check' = [check EXCEPT !.has_JSON_preferences = TRUE]

NextHasNoJsonPreferences ==
  /\ NecessaryJsonPreferences
  /\ directory' = [directory EXCEPT !.has_JSON_preferences = FALSE]
  /\ check'     = [check     EXCEPT !.has_JSON_preferences = TRUE]
...
NoDeadlockNextHasNoJsonPreferences ==
  /\ AllChecked
  /\ isTrue(1)
  /\ isTrue(2)
  /\ hasBeenDisproved(3)
...
(*===========================================================================*)
(*  Step-level (relating current and primed state) properties used by the   *)
(*  monotonicity theorems T6 and T7.                                        *)
(*===========================================================================*)
...
<1>2. Inv /\ [Next]_vars => Inv'
  ...
  <2>1. CASE NextHasDir
    ...
    <3>f. ChainOrderCheck'
      BY <3>b, <3>c DEF ChainOrderCheck, hasBeenChecked,
                        NothingHasBeenChecked, hasNotBeenChecked
    <3>g. Soundness'
  ...
  <2>3. CASE NextHasPreferencesDir
    ...
    <3>d. hasNotBeenChecked(2)
      BY <2>3 DEF NextHasPreferencesDir, NecessaryConditionPreferencesDir
    <3>e. NothingHasBeenDisproved
      BY <2>3 DEF NextHasPreferencesDir, NecessaryConditionPreferencesDir
    <3>f. hasNotBeenChecked(3)
    ...
    <3>h. ChainOrderCheck'
      BY <3>b, <3>c, <3>f DEF ChainOrderCheck, hasBeenChecked,
                              hasNotBeenChecked
    <3>i. Soundness'
      BY <3>a, <3>b, <3>e, <3>f DEF Soundness, hasBeenChecked,
                                    hasNotBeenChecked, isTrue,
                                    NothingHasBeenDisproved
  ...
  <2>5. CASE NextHasJsonPreferences
    ...
    <3>b. check' = [check EXCEPT !.has_JSON_preferences = TRUE]
      BY <2>5 DEF NextHasJsonPreferences
    <3>c. hasBeenChecked(1)
      BY <2>5 DEF NextHasJsonPreferences, NecessaryJsonPreferences
  ...
  <2>6. CASE NextHasNoJsonPreferences
    <3>a. directory' = [directory EXCEPT !.has_JSON_preferences = FALSE]
      BY <2>6 DEF NextHasNoJsonPreferences
    <3>b. check' = [check EXCEPT !.has_JSON_preferences = TRUE]
      BY <2>6 DEF NextHasNoJsonPreferences
    <3>c. hasBeenChecked(1)
      BY <2>6 DEF NextHasNoJsonPreferences, NecessaryJsonPreferences
    <3>d. hasBeenChecked(2)
      BY <2>6 DEF NextHasNoJsonPreferences, NecessaryJsonPreferences
    <3>e. NothingHasBeenDisproved
      BY <2>6 DEF NextHasNoJsonPreferences, NecessaryJsonPreferences
    ...
    <3>i. DisprovalImpliesChecked'
      BY <3>a, <3>b, <3>e DEF DisprovalImpliesChecked,
                              ...
                              isTrue, NothingHasBeenDisproved
    <3>j. AtMostOneDisproved'
      BY <3>a, <3>e DEF AtMostOneDisproved, AtLeastTwoDisproved, hasBeenDisproved, AtLeastTwoDisproved, 
                        isTrue, NothingHasBeenDisproved
    <3>k. QED
      BY <3>f, <3>g, <3>h, <3>i, <3>j DEF Inv
...
<1>1. [Next]_vars => [DirectoryMonotoneStep]_vars
  ...
  <2>4. CASE NextHasNoPreferencesDir
    BY <2>4 DEF NextHasNoPreferencesDir,
                ...
                NothingHasBeenDisproved
  <2>5. CASE NextHasJsonPreferences
    BY <2>5 DEF NextHasJsonPreferences, DirectoryMonotoneStep,
                hasBeenDisproved, isTrue
  <2>6. CASE NextHasNoJsonPreferences
    BY <2>6 DEF NextHasNoJsonPreferences, NecessaryJsonPreferences,
                DirectoryMonotoneStep, hasBeenDisproved, isTrue,
...
<1>1. [Next]_vars => [CheckMonotoneStep]_vars
  ...
  <2>4. CASE NextHasNoPreferencesDir
    BY <2>4 DEF NextHasNoPreferencesDir, CheckMonotoneStep,
                hasBeenChecked
  <2>5. CASE NextHasJsonPreferences
    BY <2>5 DEF NextHasJsonPreferences, CheckMonotoneStep,
                hasBeenChecked
  <2>6. CASE NextHasNoJsonPreferences
    BY <2>6 DEF NextHasNoJsonPreferences, CheckMonotoneStep,
                hasBeenChecked
  <2>7. CASE NoDeadlockNext
    BY <2>7 DEF NoDeadlockNext, NextNoDeadLockCriterion,
