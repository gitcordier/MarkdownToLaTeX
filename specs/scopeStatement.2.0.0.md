...
Invoked as `markdowntolatex <input.md> [options]`, the binary writes a `.tex` file or a `.pdf` file to disk, according to the requested pipeline. Options select the LaTeX engine, the user-preferences JSON file, and the output path.
...
6. **output configuration, LaTeX settings**: Provided by a JSON file that MUST be named preferences.json . Path MUST be $DIR/preferences/preferences.json
7. **Preferences schema.** What keys does `preferences.json` accept, and what are the defaults: see preferences file section below

## Preferences file

A preferences.json MUST have the same keys than the default preferences.json, see below:
